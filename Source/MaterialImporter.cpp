...
std::string MaterialImporter::importTexture(
    int texIndex, const tinygltf::Model& model,
...
bool MaterialImporter::Import(
    const tinygltf::Material& gltfMat, const tinygltf::Model& model,
    ...
    int /*materialIndex*/, const std::string& basePath)
{
    ...
    // metallic-roughness texture - B channel = metallic, G channel = roughness (glTF spec)
...
bool MaterialImporter::Load(const std::string& file, std::unique_ptr<Material>& outMaterial)
{
    ...
    loadTex(metalRoughPath, &Material::setMetalRoughMap);  // was missing entirely

    return true;
}

bool MaterialImporter::Save(
    const MaterialHeader& header,
    ...
    const std::string& file)
{
    ...
    payload.reserve(baseColorPath.size() + normalPath.size() +
        aoPath.size() + emissivePath.size() + metalRoughPath.size());
    ...
    append(aoPath);
    append(emissivePath);
    append(metalRoughPath);  // was missing entirely
