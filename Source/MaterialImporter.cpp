    ...
    TextureImporter::TextureType type)
{
    ...
    // ? Handle embedded textures (not supported yet)
    if (image.uri.empty()) {
        if (!image.image.empty()) {
            LOG("MaterialImporter: Embedded texture detected but not supported yet");
        }
        return {};
    }
    ...
    // ? Unique filename using hash (prevents collisions)
    std::filesystem::path fullPath = std::filesystem::path(basePath) / uri;
    size_t hash = std::hash<std::string>{}(fullPath.string());
    std::string safeName = std::to_string(hash);

    std::string ddsPath = matFolder + safeName + ".dds";
    std::string metaPath = ImporterUtils::MetaPath(ddsPath);

    // ? Cache check (important)
    if (!fs->Exists(ddsPath.c_str()) || !fs->Exists(metaPath.c_str())) {
        LOG("MaterialImporter: Importing texture %s", fullPath.string().c_str());

        if (!TextureImporter::Import(fullPath.string().c_str(), ddsPath, type)) {
            LOG("MaterialImporter: Failed to import texture %s", fullPath.string().c_str());
            return {};
        }
    }
    ...
    int /*materialIndex*/, const std::string& basePath)
{
    ...
    // Base Color (Albedo)
    std::string baseColorPath = importTexture(
        pbr.baseColorTexture.index, model, sceneName, basePath,
    ...
    // Normal
    std::string normalPath = importTexture(
        gltfMat.normalTexture.index, model, sceneName, basePath,
    ...
    // AO
    std::string aoPath = importTexture(
        gltfMat.occlusionTexture.index, model, sceneName, basePath,
    ...
    // Emissive
    std::string emissivePath = importTexture(
        gltfMat.emissiveTexture.index, model, sceneName, basePath,
    ...
    // Metallic-Roughness
    std::string metalRoughPath = importTexture(
        pbr.metallicRoughnessTexture.index, model, sceneName, basePath,
        TextureImporter::TextureType::OcclusionMetalRough);
...
bool MaterialImporter::Load(const std::string& file, std::unique_ptr<Material>& outMaterial)
{
    ...
    std::string normalPath, aoPath, emissivePath, metalRoughPath;
