...
{
    // Root layout:
    //   slot 0: CBV b0  - SkyboxCB (vp, flipX, flipZ)
    //   slot 1: SRV t0  - cubemap texture
    //   slot 2: Sampler s0-s3
    ...
    {
        LOG("SkyboxRenderer: root signature serialise failed: %s",
...
{
    // SkyboxVS.cso — reads POSITION, outputs TEXCOORD (cube dir) + SV_Position
    // SkyboxPS.cso — samples TextureCube, applies gamma correction
    ...
    // Skybox geometry outputs clipPos.xyww so depth == 1.0; use LESS_EQUAL so it passes
...
void SkyboxRenderer::render(ID3D12GraphicsCommandList* cmd, const EnvironmentMap& env,
    const Matrix& view, const Matrix& projection)
{
    if (!env.isValid())
        return;

    // Strip translation from the view matrix so the skybox stays fixed at the horizon
