...
float4 main(float3 worldPos : POSITION, float2 texCoord : TEXCOORD, float3 normal : NORMAL0, float3 tangent : TANGENT) : SV_TARGET
{
    ...
    float metallic;
    SampleMetallicRoughness(InstanceMaterial, BaseColorTex, MetallicRoughnessTex, texCoord, baseColour, roughness, alphaRoughness, metallic);
    ...
    SampleAmbientOcclusion(InstanceMaterial, OcclusionTex, texCoord, NdotV, NdotR, alphaRoughness, diffuseAO, specularAO);
        
    float3 colour = ComputeIBLLighting(V, N, IrradianceMap, PrefilteredEnvMap, BrdfLUT, EnvRoughnessLevels, baseColour, roughness, metallic, diffuseAO, specularAO);
    
    for (uint i = 0; i < DirLightCount; i++)
        colour += EvaluateDirectionalLight(V, N, DirLights[i], baseColour, alphaRoughness, metallic);
    
    for (uint i = 0; i < PointLightCount; i++) 
        colour += EvaluatePointLight(V, N, PointLights[i], worldPos, baseColour, alphaRoughness, metallic);

    for (uint i = 0; i < SpotLightCount; i++)
        colour += EvaluateSpotLight(V, N, SpotLights[i], worldPos, baseColour, alphaRoughness, metallic);
