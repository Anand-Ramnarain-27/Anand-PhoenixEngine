...
float3 ComputeIBLLighting(in float3 V, in float3 N, in TextureCube irradianceMap, in TextureCube prefilteredEnvMap, in Texture2D brdfLUT, in uint roughnessLevels, in float3 baseColor, in float roughness, in float metallic, in float diffuseAO, in float specularAO) { 
    ...
    float3 scaledRadiance, biasedRadiance; SampleSpecularIBL(R, NdotV, roughness, 
    float(roughnessLevels), prefilteredEnvMap, brdfLUT, scaledRadiance, biasedRadiance); 
