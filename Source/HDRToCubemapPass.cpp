...
bool HDRToCubemapPass::recordMipLevel(ID3D12Device* device, ID3D12GraphicsCommandList* cmd, EnvironmentMap& env, uint32_t mipIndex) {
    ...
    if (!mipTable.isValid()) {
        LOG("HDRToCubemapPass: failed to alloc mip SRV table (mip=%u)", mipIndex);
        return false;
    }
...
bool HDRToCubemapPass::createConversionPipeline(ID3D12Device* device) {
    ...
    D3D12_INPUT_ELEMENT_DESC layout = { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
...
void HDRToCubemapPass::renderFace(
    ID3D12GraphicsCommandList* cmd, ID3D12Resource* target,
    uint32_t faceIndex, uint32_t mipLevel, uint32_t totalMips, uint32_t baseFaceSize,
    D3D12_GPU_DESCRIPTOR_HANDLE sourceSRV, ID3D12RootSignature* rs,
    ID3D12PipelineState* pso, DXGI_FORMAT rtvFmt)
{
    ...
    XMStoreFloat4x4(&constants.vp, XMMatrixTranspose(vp_m));
    constants.flipX = FaceProjection::needsFlipX(faceIndex) ? 1 : 0;
    constants.flipZ = FaceProjection::needsFlipZ(faceIndex) ? 1 : 0;
...
void HDRToCubemapPass::blitMipFace(
    ID3D12GraphicsCommandList* cmd, ID3D12Resource* cubemap,
    uint32_t faceIndex, uint32_t dstMip, uint32_t totalMips,
