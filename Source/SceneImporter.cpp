...
bool SceneImporter::ImportFromLoadedGLTF(const tinygltf::Model& gltfModel, const std::string& sceneName) {
    ...
    std::string basePath = fs->GetAssetsPath() + "Models/" + sceneName + "/";
    ...
    for (const auto& mesh : gltfModel.meshes) {
        for (const auto& prim : mesh.primitives) {
            if (!MeshImporter::Import(
                prim,
                gltfModel,
                ImporterUtils::IndexedPath(meshFolder, meshIndex++, ".mesh")))
            {
    ...
    for (const auto& mat : gltfModel.materials) {
        MaterialImporter::Import(
            ...
            gltfModel,
            sceneName,
            ImporterUtils::IndexedPath(matFolder, matIndex++, ".mat"),
            matIndex - 1,
            basePath);
    }

    {
        std::string skinRoot = fs->GetLibraryPath() + "Skins";
        std::string skinFolder = skinRoot + "/" + sceneName;

        fs->CreateDir(skinRoot.c_str());
        fs->CreateDir(skinFolder.c_str());

        for (int si = 0; si < (int)gltfModel.skins.size(); ++si) {
            std::string skinFile = skinFolder + "/" + std::to_string(si) + ".skin";

            if (!fs->Exists(skinFile.c_str())) {
                if (!SkinImporter::Import(gltfModel, si, skinFile)) {
                    LOG("SceneImporter: Failed to import skin %d", si);
                }
            }
        }
    }
...
bool SceneImporter::SaveSceneMetadata(const std::string& sceneName, const tinygltf::Model& gltfModel) {
    ...
    header.skinCount = (uint32_t)gltfModel.skins.size();
    ...
    return ImporterUtils::SaveBuffer(
        app->getFileSystem()->GetLibraryPath() + "Meshes/" + sceneName + "/scene.meta",
        header,
        payload
    );
}
