    ...
    Document::AllocatorType& a)
{
    ...
    Value tf(kObjectType);
    {
        ...
        pos.PushBack(t->position.x, a).PushBack(t->position.y, a).PushBack(t->position.z, a);
        Value rot(kArrayType);
        rot.PushBack(t->rotation.x, a).PushBack(t->rotation.y, a)
            .PushBack(t->rotation.z, a).PushBack(t->rotation.w, a);
        Value scl(kArrayType);
        scl.PushBack(t->scale.x, a).PushBack(t->scale.y, a).PushBack(t->scale.z, a);
...
GameObject* PrefabManager::deserialiseNode(const Value& node,
    ModuleScene* scene,
    GameObject* parent)
{
    ...
    bool        active = node.HasMember("Active") ? node["Active"].GetBool() : true;
    ...
    if (node.HasMember("PrefabLink") && node["PrefabLink"].IsObject())
    {
        ...
        if (!linkData.prefabName.empty())
            PrefabManager::linkInstance(go, linkData);
    }
    ...
    if (node.HasMember("Transform") && node["Transform"].IsObject())
    {
        ...
        if (tf.HasMember("position"))
        {
            const auto& p = tf["position"];
            t->position = { p[0].GetFloat(), p[1].GetFloat(), p[2].GetFloat() };
        }
        if (tf.HasMember("rotation"))
        {
            const auto& r = tf["rotation"];
            t->rotation = { r[0].GetFloat(), r[1].GetFloat(), r[2].GetFloat(), r[3].GetFloat() };
        }
        if (tf.HasMember("scale"))
        {
            const auto& s = tf["scale"];
            t->scale = { s[0].GetFloat(), s[1].GetFloat(), s[2].GetFloat() };
        }
        t->markDirty();
    }
    ...
    if (node.HasMember("Components") && node["Components"].IsArray())
    {
        for (SizeType i = 0; i < node["Components"].Size(); ++i)
        {
            ...
            {
                LOG("PrefabManager: Could not create component type %d for '%s'",
                    cn["Type"].GetInt(), name);
            }
        }
    }
...
bool PrefabManager::createPrefab(const GameObject* go, const std::string& prefabName)
{
    ...
    {
        LOG("PrefabManager::createPrefab: null go or empty name");
        return false;
    }
    ...
    if (!fp)
    {
        LOG("PrefabManager::createPrefab: Cannot open '%s' for writing", path.c_str());
        return false;
    }
...
GameObject* PrefabManager::instantiatePrefab(const std::string& prefabName,
    ModuleScene* scene)
{
    ...
    {
        LOG("PrefabManager::instantiatePrefab: Prefab not found: %s", path.c_str());
        return nullptr;
    }
    ...
    if (!fp)
    {
        LOG("PrefabManager::instantiatePrefab: Cannot open '%s'", path.c_str());
        return nullptr;
    }
    ...
    {
        LOG("PrefabManager::instantiatePrefab: JSON parse error in '%s': %s (offset %zu)",
            path.c_str(),
    ...
    {
        LOG("PrefabManager::instantiatePrefab: Missing 'GameObject' in '%s'", path.c_str());
        return nullptr;
    }
    ...
    instData.prefabUID = doc.HasMember("PrefabUID") ? doc["PrefabUID"].GetUint()
        : makePrefabUID(prefabName);
    linkInstance(go, instData);

    LOG("PrefabManager: Instantiated prefab '%s' -> GO '%s'",
        prefabName.c_str(), go->getName().c_str());
    return go;
}
...
bool PrefabManager::applyToPrefab(const GameObject* go, bool respectOverrides)
{
    ...
    {
        LOG("PrefabManager::applyToPrefab: '%s' is not a prefab instance",
    ...
    if (!fp)
    {
        LOG("PrefabManager::applyToPrefab: Cannot write to '%s'", path.c_str());
        return false;
    }
    ...
    LOG("PrefabManager: Applied instance '%s' -> prefab '%s'",
...
bool PrefabManager::revertToPrefab(GameObject* go, ModuleScene* scene)
{
    ...
    if (!inst || inst->prefabName.empty())
    {
        LOG("PrefabManager::revertToPrefab: '%s' is not a prefab instance",
            go ? go->getName().c_str() : "null");
        return false;
    }
    ...
    if (!app->getFileSystem()->Exists(path.c_str()))
    {
        LOG("PrefabManager::revertToPrefab: Prefab file missing: %s", path.c_str());
        return false;
    }
    ...
    if (doc.HasParseError() || !doc.HasMember("GameObject")) return false;
    ...
    {
        const Value& tf = node["Transform"];
        const auto& ovTf = savedOverrides.modifiedProperties;
        auto it = ovTf.find(kTransformType);
        const std::unordered_set<std::string>* ovSet = (it != ovTf.end()) ? &it->second : nullptr;

        auto overridden = [&](const char* prop) -> bool
            {
                return ovSet && ovSet->count(prop) > 0;
            };

        if (!overridden("position") && tf.HasMember("position"))
        {
            const auto& p = tf["position"];
            t->position = { p[0].GetFloat(), p[1].GetFloat(), p[2].GetFloat() };
        }
        if (!overridden("rotation") && tf.HasMember("rotation"))
        {
            const auto& r = tf["rotation"];
            t->rotation = { r[0].GetFloat(), r[1].GetFloat(), r[2].GetFloat(), r[3].GetFloat() };
        }
        if (!overridden("scale") && tf.HasMember("scale"))
        {
            const auto& s = tf["scale"];
            t->scale = { s[0].GetFloat(), s[1].GetFloat(), s[2].GetFloat() };
        }
    ...
    if (node.HasMember("Components") && node["Components"].IsArray())
    {
        for (SizeType i = 0; i < node["Components"].Size(); ++i)
        {
            ...
            if (std::find(removedVec.begin(), removedVec.end(), typeInt) != removedVec.end())
                continue;

            Component* existing = nullptr;
            for (const auto& c : go->getComponents())
                if (c->getType() == type) { existing = c.get(); break; }
            ...
            else
            {
                ...
                bool fullyOverridden = (it != savedOverrides.modifiedProperties.end()
                    && it->second.count("__all__") > 0);
                if (!fullyOverridden)
                    existing->onLoad(cn["Data"].GetString());
            }
        }
    }
    ...
    LOG("PrefabManager: Reverted '%s' from prefab '%s'",
        go->getName().c_str(), inst->prefabName.c_str());
    return true;
}

void PrefabManager::markPropertyOverride(GameObject* go,
    int                componentType,
...
bool PrefabManager::createVariant(const std::string& srcPrefabName,
    const std::string& dstPrefabName)
{
    if (srcPrefabName.empty() || dstPrefabName.empty()) return false;

    std::string srcPath = getPrefabPath(srcPrefabName);
    std::string dstPath = getPrefabPath(dstPrefabName);

    if (!app->getFileSystem()->Exists(srcPath.c_str()))
    {
        LOG("PrefabManager::createVariant: Source not found: %s", srcPath.c_str());
        return false;
    }
    ...
    auto& a = doc.GetAllocator();
    if (doc.HasMember("PrefabName"))
        doc["PrefabName"].SetString(dstPrefabName.c_str(), a);
    else
        doc.AddMember("PrefabName", Value(dstPrefabName.c_str(), a), a);
    ...
    if (doc.HasMember("VariantOf"))
        doc["VariantOf"].SetString(srcPrefabName.c_str(), a);
    else
        doc.AddMember("VariantOf", Value(srcPrefabName.c_str(), a), a);

    FILE* out = fopen(dstPath.c_str(), "wb");
    if (!out) return false;
    FileWriteStream os(out, buf, sizeof(buf));
    PrettyWriter<FileWriteStream> writer(os);
    writer.SetIndent(' ', 2);
    doc.Accept(writer);
    fclose(out);

    LOG("PrefabManager: Created variant '%s' from '%s'",
...
std::vector<PrefabManager::PrefabInfo> PrefabManager::listPrefabsInfo()
{
    ...
    try
    {
        for (const auto& entry : fs::directory_iterator(kPrefabDir))
        {
            ...
            if (entry.path().extension() != kPrefabExt) continue;
            ...
            if (doc.HasMember("GameObject") && doc["GameObject"].IsObject())
            {
                ...
                compNames.push_back("Transform");
                if (gn.HasMember("Components") && gn["Components"].IsArray())
                {
                    for (SizeType i = 0; i < gn["Components"].Size(); ++i)
                    {
                        int t = gn["Components"][i]["Type"].GetInt();
                        switch (static_cast<Component::Type>(t))
                        {
...
{
    return app->getFileSystem()->Exists(getPrefabPath(prefabName).c_str());
}

void PrefabManager::linkInstance(GameObject* go,
    const PrefabInstanceData& data)
{
    if (!go) return;
    registry()[go] = data;
}
