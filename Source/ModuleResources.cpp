...
void ModuleResources::registerAnimation(UID uid, const std::string& path) { m_registry[uid] = { path, ResourceBase::Type::Animation, 0 }; }
...
ResourceBase* ModuleResources::CreateResourceFromUID(UID uid) {
    ...
    if (regIt != m_registry.end()) {
        ...
        case ResourceBase::Type::Animation: { auto* r = new ResourceAnimation(uid); r->libraryFile = rec.libraryPath; r->assetsFile = assetPath; AnimImporter::Load(rec.libraryPath, *r); return r; }
