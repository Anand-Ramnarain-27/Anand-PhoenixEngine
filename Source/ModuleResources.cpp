...
void ModuleResources::registerAnimation(
    UID uid, const std::string& libPath)
{
    m_registry[uid] = { libPath, ResourceBase::Type::Animation, 0 };
}
...
ResourceAnimation* ModuleResources::RequestAnimation(UID uid) {
    return static_cast<ResourceAnimation*>(RequestResource(uid));
}
...
ResourceBase* ModuleResources::CreateResourceFromUID(UID uid) {
    ...
    if (regIt != m_registry.end()) {
        ...
        case ResourceBase::Type::Animation: {
