...
bool EnvironmentSystem::init(ID3D12Device* device, DXGI_FORMAT rtvFormat, DXGI_FORMAT dsvFormat, bool useMSAA){
	return m_renderer.init(device, useMSAA);
}

void EnvironmentSystem::load(const std::string& file){
	m_environment = loadCubemap(file);
}

void EnvironmentSystem::loadHDR(const std::string& hdrFile, uint32_t cubeFaceSize){
	m_environment = loadHDRMap(hdrFile, cubeFaceSize);
}

void EnvironmentSystem::render(ID3D12GraphicsCommandList* cmd, const Matrix& view, const Matrix& projection){
	if (m_environment && m_environment->isValid())
		m_renderer.render(cmd, *m_environment, view, projection);
}

std::unique_ptr<EnvironmentMap> EnvironmentSystem::loadCubemap(const std::string& file){
...
std::unique_ptr<EnvironmentMap> EnvironmentSystem::loadHDRMap(const std::string& hdrFile, uint32_t cubeFaceSize){
	auto* d3d12 = app->getD3D12();
	auto* resources = app->getGPUResources();
	auto* shaderDesc = app->getShaderDescriptors();
...
std::unique_ptr<EnvironmentMap> EnvironmentSystem::bakeIBL(ModuleD3D12* d3d12, ModuleShaderDescriptors* shaderDesc, std::unique_ptr<EnvironmentMap> env){
	auto* samplerHeap = app->getSamplerHeap();
