...
static constexpr UINT MAT_SLOT_BASECOLOR = 0; // t6
static constexpr UINT MAT_SLOT_METALROUGH = 1; // t7
static constexpr UINT MAT_SLOT_NORMAL = 2; // t8
static constexpr UINT MAT_SLOT_AO = 3; // t9
static constexpr UINT MAT_SLOT_EMISSIVE = 4; // t10
...
namespace
{
        ...
        void** mapped, const wchar_t* name)
    {
        ...
        if (FAILED(hr)) { LOG("MeshRenderPass: upload buf failed 0x%08X", hr); return nullptr; }
        buf->SetName(name);
    ...
    // Write a 1x1x6 TextureCube SRV into a ShaderTableDesc slot.
    // Used for the fallback IBL cubemap slots (t3, t4) when no skybox is loaded.
...
{
    if (!m_pipeline.init(device, useMSAA)) { LOG("MeshRenderPass: pipeline init failed"); return false; }
...
bool MeshRenderPass::createUploadBuffers(ID3D12Device* device)
{
    ...
    m_mvpRing = makeUploadBuf(device, (UINT64)mvpSz * MAX_INSTANCES,
        &m_mvpMapped, L"MeshPass_MVPRing");
    if (!m_mvpRing) return false;

    m_perFrameCB = makeUploadBuf(device,
        cbAlign(sizeof(MeshPipeline::CbPerFrame)),
        &m_perFrameMapped, L"MeshPass_PerFrameCB");
    if (!m_perFrameCB) return false;

    m_perInstanceRing = makeUploadBuf(device,
        (UINT64)instSz * MAX_INSTANCES,
        &m_perInstanceMapped, L"MeshPass_InstanceRing");
...
{
    // 1x1 Texture2D ? used for material texture slots that a mesh doesn't have
    {
        ...
        if (FAILED(hr)) { LOG("MeshRenderPass: fallback Texture2D failed 0x%08X", hr); return false; }
        m_fallbackTex2D->SetName(L"MeshPass_Fallback2D");
    }

    // 1x1x6 TextureCube ? used for IBL cubemap slots when no skybox is loaded.
    // Sampling this returns (0,0,0) which means no IBL contribution ? the mesh
    // will appear dark until a skybox is loaded, which is the correct behaviour.
    {
        D3D12_RESOURCE_DESC td = {};
        td.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
        td.Width = td.Height = 1;
        td.DepthOrArraySize = 6;  // 6 faces = cubemap
        td.MipLevels = 1;
        td.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
        td.SampleDesc = { 1, 0 };
        ...
        auto hp = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
        HRESULT hr = device->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &td,
            D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, nullptr, IID_PPV_ARGS(&m_fallbackCube));
        if (FAILED(hr)) { LOG("MeshRenderPass: fallback TextureCube failed 0x%08X", hr); return false; }
        ...
        // BRDF LUT is Texture2D, not a cubemap
...
void MeshRenderPass::uploadLights(const FrameLightData& lights)
{
    ...
    copy(m_dirLightMapped, lights.dirLights.data(),
        lights.dirLights.size(), sizeof(MeshPipeline::GPUDirectionalLight), MeshPipeline::MAX_DIR_LIGHTS);
    copy(m_pointLightMapped, lights.pointLights.data(),
        lights.pointLights.size(), sizeof(MeshPipeline::GPUPointLight), MeshPipeline::MAX_POINT_LIGHTS);
    copy(m_spotLightMapped, lights.spotLights.data(),
        lights.spotLights.size(), sizeof(MeshPipeline::GPUSpotLight), MeshPipeline::MAX_SPOT_LIGHTS);
}

void MeshRenderPass::uploadPerFrameCB(const FrameLightData& lights,
...
void MeshRenderPass::writePerDrawCBs(const MeshEntry& entry, const Matrix& viewProj,
    UINT slot,
    ...
    ModuleSamplerHeap* samplerHeap)
{
    ...
    // ?? CRITICAL FIX: compute roughness levels from environment
    ...
    cmd->SetGraphicsRootConstantBufferView(
        MeshPipeline::SLOT_PERFRAME_CB,
        m_perFrameCB->GetGPUVirtualAddress());

    // Bind light buffers
    cmd->SetGraphicsRootDescriptorTable(
        MeshPipeline::SLOT_DIR_LIGHTS,
        m_dirLightSRV.getGPUHandle(0));

    cmd->SetGraphicsRootDescriptorTable(
        MeshPipeline::SLOT_POINT_LIGHTS,
        m_pointLightSRV.getGPUHandle(0));

    cmd->SetGraphicsRootDescriptorTable(
        MeshPipeline::SLOT_SPOT_LIGHTS,
        m_spotLightSRV.getGPUHandle(0));

    // ?? IBL binding (real or fallback)
    if (env && env->hasIBL())
    {
    ...
    {
        cmd->SetGraphicsRootDescriptorTable(
            MeshPipeline::SLOT_IRRADIANCE,
            m_fallbackIrradianceSRV.getGPUHandle(0));

        cmd->SetGraphicsRootDescriptorTable(
            MeshPipeline::SLOT_PREFILTER,
            m_fallbackPrefilterSRV.getGPUHandle(0));

        cmd->SetGraphicsRootDescriptorTable(
            MeshPipeline::SLOT_BRDF_LUT,
    ...
    for (MeshEntry* entry : meshes)
    {
        ...
        cmd->SetGraphicsRootConstantBufferView(
            MeshPipeline::SLOT_MVP_CB, mvpVA);

        cmd->SetGraphicsRootConstantBufferView(
            MeshPipeline::SLOT_PERINSTANCE_CB, instVA);

        // Material table
        ShaderTableDesc& matTable = m_matRing[slot];

        // Reset to fallback textures
        ...
        // Apply real material textures if present
        const Material* mat = nullptr;
        if (entry->materialRes) mat = entry->materialRes->getMaterial();
        else if (entry->material) mat = entry->material;
