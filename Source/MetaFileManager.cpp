...
bool MetaFileManager::save(const std::string& assetPath, const MetaData& meta) {
    ...
    StringBuffer sb; PrettyWriter<StringBuffer> writer(sb); doc.Accept(writer);
...
UID MetaFileManager::getOrCreateUID(const std::string& assetPath, ResourceBase::Type type) {
    ...
    meta.uid = generateUID(); meta.type = type; meta.lastModified = getLastModified(assetPath);
    save(assetPath, meta);
...
uint64_t MetaFileManager::getLastModified(const std::string& filePath) {
    try { return (uint64_t)std::chrono::duration_cast<std::chrono::seconds>(fs::last_write_time(filePath).time_since_epoch()).count(); }
    catch (...) { return 0; }
}