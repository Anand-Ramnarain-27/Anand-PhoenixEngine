...
private:
    ...
    bool createCubemapResource(ID3D12Device* device, uint32_t size, uint32_t mips, DXGI_FORMAT fmt, const wchar_t* name, ComPtr<ID3D12Resource>& out);
    bool create2DResource(ID3D12Device* device, uint32_t size, DXGI_FORMAT fmt, const wchar_t* name, ComPtr<ID3D12Resource>& out);
    bool createConvPipeline(ID3D12Device* device, const wchar_t* psCsoPath, DXGI_FORMAT rtvFmt, ComPtr<ID3D12RootSignature>& outRS, ComPtr<ID3D12PipelineState>& outPSO);
    bool createBRDFPipeline(ID3D12Device* device, ComPtr<ID3D12RootSignature>& outRS, ComPtr<ID3D12PipelineState>& outPSO);
    void renderCubeFace(ID3D12Device* device, ID3D12GraphicsCommandList* cmd, ID3D12Resource* target, uint32_t faceIndex, uint32_t mipLevel, uint32_t totalMips, uint32_t baseFaceSize, float roughness, ID3D12RootSignature* rs, ID3D12PipelineState* pso, D3D12_GPU_DESCRIPTOR_HANDLE sourceSRV, DXGI_FORMAT rtvFmt);
