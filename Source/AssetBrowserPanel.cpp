    ...
    else
    {
        ...
        if (e.ext == ".json")
        {
            if (ImGui::MenuItem("Load Scene"))
                if (auto* sm = m_editor->getSceneManager(); sm && sm->loadScene(e.path))
...
{
    if (ImGui::IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem) && ImGui::IsMouseClicked(ImGuiMouseButton_Right) && !ImGui::IsAnyItemHovered())
...
void AssetBrowserPanel::spawnAsset(const std::string& path)
{
    ...
    {
        if (auto* sm = m_editor->getSceneManager(); sm && sm->loadScene(path))
...
void AssetBrowserPanel::prefabApply()
{
    ...
    m_editor->log(ok ? "Applied to prefab." : "Apply failed.", ok ? EditorColors::Success : EditorColors::Danger);
}
...
void AssetBrowserPanel::prefabRevert()
{
    ...
    m_editor->log(ok ? "Reverted." : "Revert failed.", ok ? EditorColors::Success : EditorColors::Danger);
}
...
void AssetBrowserPanel::prefabCreateVariant(const std::string& src, const std::string& dst)
{
    ...
    m_editor->log(ok ? ("Variant: " + dst).c_str() : "Variant failed.", ok ? EditorColors::Success : EditorColors::Danger);
    m_dirty = true;
}
