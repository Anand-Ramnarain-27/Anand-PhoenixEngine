...
void Mesh::setData(ID3D12GraphicsCommandList* cmd, ModuleStaticBuffer* staticBuffer,const std::vector<Vertex>& vertices, const std::vector<uint32_t>& indices, int materialIndex){
    ...
    if (!staticBuffer || vertices.empty()){
        LOG("Mesh::setData(pool): invalid args — skipping GPU upload");
        computeAABB();
        return;
    }
    ...
    if (!indices.empty()){
        ...
        m_indexBufferView = staticBuffer->allocIndexBuffer(cmd, indices.data(), ibSize, DXGI_FORMAT_R32_UINT, "MeshIB");
...
void Mesh::uploadToGPU(ID3D12GraphicsCommandList* cmd, ModuleStaticBuffer* staticBuffer){
    ...
    if (m_hasVertexBuffer) return; 
    if (!staticBuffer || m_vertices.empty()) return;

    const size_t vbSize = m_vertices.size() * sizeof(Vertex);
    m_vertexBufferView = staticBuffer->allocVertexBuffer(cmd, m_vertices.data(), vbSize, sizeof(Vertex), "MeshVB");
    m_hasVertexBuffer = (m_vertexBufferView.BufferLocation != 0);

    if (!m_indices.empty()){
        const size_t ibSize = m_indices.size() * sizeof(uint32_t);
        m_indexBufferView = staticBuffer->allocIndexBuffer(cmd, m_indices.data(), ibSize, DXGI_FORMAT_R32_UINT, "MeshIB");
        m_hasIndexBuffer = (m_indexBufferView.BufferLocation != 0);
    }
}
...
void Mesh::draw(ID3D12GraphicsCommandList* cmdList) const{
    ...
    if (!m_legacyVertexBuffer)
        const_cast<Mesh*>(this)->createLegacyBuffers();
...
void Mesh::createLegacyBuffers()
{
    ...
    m_legacyVertexBuffer = gpu->createDefaultBuffer(
        m_vertices.data(), m_vertices.size() * sizeof(Vertex), "MeshVB_Legacy");

    if (!m_indices.empty())
        m_legacyIndexBuffer = gpu->createDefaultBuffer(
            m_indices.data(), m_indices.size() * sizeof(uint32_t), "MeshIB_Legacy");
}
