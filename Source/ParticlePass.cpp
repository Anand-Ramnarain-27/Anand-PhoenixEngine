...
{
    // Each slot holds EITHER a CbParticle OR a CbParticleUpdate — both fit in 256 bytes.
    // Allocate enough for MAX_EMITTERS × 2 (one gfx CB + one cs CB per emitter).
                                                                ...
                                                                size_t key, UINT capacity)
{
    ...
    // Remove old buffers if capacity changed.
    ...
    // ---- Upload heap buffer (CPU writes, VS reads directly for no-CS path) ----
    {
        auto hp = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
        ...
        // SRV for the upload buffer (used by render VS when CS is not active).
    ...
    // ---- Default heap UAV buffer (CS writes, VS reads after barrier) -----------
    {
        ...
        // SRV for the UAV buffer (render VS reads this after CS+barrier).
                           ...
                           uint32_t width, uint32_t height)
{
    ...
    UINT       cbSlot      = 0;   // index into m_cbRing
    UINT       drawCount   = 0;

    // ---- Sort: alpha before additive (back-to-front within each group is the
    //           caller's responsibility via the sorted request vector order) ----
    ...
    for (const auto& req : requests) {
        ...
        if (req.gpuTurbulence && eb.uavBuf && eb.uavUAV.isValid() && eb.uploadSRV.isValid()) {
            ...
            // Upload CbParticleUpdate into the CB ring.
            ...
            // UAV barrier — wait for CS writes before transitioning for render.
            D3D12_RESOURCE_BARRIER uavBarrier = CD3DX12_RESOURCE_BARRIER::UAV(eb.uavBuf.Get());
            cmd->ResourceBarrier(1, &uavBarrier);

            // Transition UAV buffer to SRV state for the render VS.
        ...
        // Per-draw CB (ViewProj, camera axes).
        CbParticle cb = {};
        ...
        // 4 verts per particle (TRIANGLESTRIP), liveCount instances.
        cmd->DrawInstanced(4, liveCount, 0, 0);
        ++cbSlot;
        ++drawCount;

        // 4. If CS was used, transition UAV buffer back to UAV state for next frame.
